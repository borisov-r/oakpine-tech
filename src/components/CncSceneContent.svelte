<script lang="ts">
  ...
  // Larger values improve tap / drag targeting on touch screens.
  const SHAFT_RADIUS = 0.08;
  const SHAFT_HEIGHT = 0.45;
  const HEAD_RADIUS = 0.20;
  const HEAD_HEIGHT = 0.30;
  ...
  function onPointerUp() {
    ...
    // If the pointer was released without a significant drag, treat it as a tap/click.
    // This makes single-step adjustment work reliably on touch devices where a
    // synthetic 'click' event may not be generated by the browser after a WebGL touch.
    if (!hadSignificantDrag && dragAdjust) {
      dragAdjust(dragClickDir);
      // Set the flag so that any subsequent Threlte-generated onclick is suppressed
      // and doesn't double-count the adjustment.
      hadSignificantDrag = true;
    }
  ...
  // Suppress the Threlte-generated onclick that may fire after a drag or after the
  // tap has already been handled in onPointerUp above. The flag is cleared after
  // the first suppressed click.
...
{#if selected}
  ...
  <T.Mesh
    ...
    onpointerdown={(e) => startArrowDrag(onAdjustX, 'x', 1, e)}
  >
  ...
  <T.Mesh
    ...
    onpointerdown={(e) => startArrowDrag(onAdjustY, 'y', 1, e)}
  >
    <T.CylinderGeometry args={[SHAFT_RADIUS, SHAFT_RADIUS, SHAFT_HEIGHT, 8]} />
    <T.MeshStandardMaterial color={ARROW_COLOR} />
  </T.Mesh>
  ...
  <T.Mesh
    ...
    onpointerdown={(e) => startArrowDrag(onAdjustZ, 'x', 1, e)}
  >
    <T.CylinderGeometry args={[SHAFT_RADIUS, SHAFT_RADIUS, SHAFT_HEIGHT, 8]} />
    <T.MeshStandardMaterial color={ARROW_COLOR} />
  </T.Mesh>
